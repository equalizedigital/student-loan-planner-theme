		...
		document.querySelectorAll('.dropdown-li').forEach(function (element) {
			...
			element.addEventListener('keydown', function (event) {
				if (event.keyCode === 13) {
					...
					// Get the value of the data-target attribute
					const targetClass =
						event.target.getAttribute('data-resourcelink');
					// Remove active class from all items before adding to the new one
					tabButtons.forEach((btn) => btn.classList.remove('active'));
		...
		function moveFocusToPreviousTabDropdown(event) {
			...
			if (
				...
				if (currentIndex - 1 !== -1) {
					...
					const focusItem = document.getElementById(
						'resource-links-dropdown'
					);
					focusItem.focus();
				}
...
document.addEventListener('DOMContentLoaded', function () {
	function footerMenuFunctions() {
		...
		widgetTitles.forEach(function (title) {
			const menuFooterContainer = title.nextElementSibling;
			if (
				menuFooterContainer &&
	...
	const widgetTitlesRespo = document.querySelectorAll('.widget-title');
...
jQuery(function ($) {
	...
	const focusableElementsString =
		'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, video, object, embed, *[tabindex], *[contenteditable]';
	...
	function showModal(obj) {
		...
		const video = jQuery('#' + obj)
			.find('.video-modal-autoplay')
			.get(0);
		if (video) {
			video.play();
		}
		...
		podcastlinkblockButtons.forEach((button, index) => {
			...
			button.addEventListener('keydown', function (e) {
				...
				if (e.keyCode === 36) {
					...
					podcastlinkblockButtons[targetIndex].removeAttribute(
						'tabindex'
					);
					podcastlinkblockButtons[targetIndex].setAttribute(
						'aria-selected',
						'true'
					);
					const contentHome = document.getElementById(
						podcastlinkblockButtons[targetIndex].getAttribute(
							'aria-controls'
						)
					);
					...
					const contentEnd = document.getElementById(
						podcastlinkblockButtons[targetIndex].getAttribute(
							'aria-controls'
						)
					);
				...
				if (e.keyCode === 37) {
					// Left Arrow
					targetIndex =
						(index - 1 + podcastlinkblockButtons.length) %
						podcastlinkblockButtons.length;
					removeAllActiveState();
					podcastlinkblockButtons[targetIndex].focus();
					podcastlinkblockButtons[targetIndex].removeAttribute(
						'tabindex'
					...
					const contentPrev = document.getElementById(
						podcastlinkblockButtons[targetIndex].getAttribute(
							'aria-controls'
						)
					);
					...
					const contentNext = document.getElementById(
						podcastlinkblockButtons[targetIndex].getAttribute(
							'aria-controls'
						)
					);
...
window.addEventListener('load', function () {
	...
	function teamHighlightFunctionality(windowWidth) {
		const highlightButton = document.querySelector(
			'.team-hightlight-block-container-team-hightlight__load_more button'
		);
		const highlightButtonText = document.querySelector(
			'.team-hightlight-block-container-team-hightlight__load_more button .text'
		);
		...
		if (highlightButton) {
			...
			if (items.length >= 6) {
				if (windowWidth <= 768) {
					...
					!document.querySelector('.team-hightlight-block-styling-1')
				) {
...
document.addEventListener('DOMContentLoaded', function () {
	...
	function teamLenderFunctionality(windowWidth) {
		const highlightButtonLender = document.querySelector(
		...
		if (highlightButtonLender) {
			const items = document.querySelectorAll('.data-tr');
			const initText = highlightButtonLenderText;
			let tabOpen = false;
...
window.addEventListener('DOMContentLoaded', () => {
	...
	if (mainTocContainer.length > 0 || tocContainerEntryContent.length > 0) {
		...
		window.addEventListener('scroll', function () {
			...
			const heroElement = document.querySelector('.inner-hero');
			if (heroElement) {
	...
	if (tabletChevron.length > 0) {
		document
			...
			.addEventListener('click', function () {
				...
				const nextItem = items[activeIndex + 1] || items[0]; // Loop back to first if at the end
...
document.addEventListener('DOMContentLoaded', function () {
	...
	if (videoCarouselElement.length) {
		const videoCarousel = new Swiper('.video-carousel-swiper-container', {
			// Optional parameters
			loop: true,
				...
				setTimeout(() => {
					if (isKeyboardNavigation) {
						videoCarousel.slides.forEach((slide) =>
							slide.classList.add('keyboard-focused')
						);
						isKeyboardNavigation = true;
					} else {
						videoCarousel.slides.forEach((slide) =>
		...
		function isTargetElement(element) {
			...
			return (
				element.classList.contains('image-placeholder-action') ||
...
document.addEventListener('DOMContentLoaded', function () {
	const videoBlockElement = document.querySelectorAll(
	...
	if (videoBlockElement.length) {
		...
		const allimageObject = document.querySelectorAll('.image-placeholder');
		const documentimageobject = document.querySelectorAll('.image-object');
		...
		function videoFunctionality(button) {
			...
			const currentImagePlaceholder =
				slideContainer.querySelector('.image-placeholder');
			const video = slideContainer.querySelector('video');
			const currentimageobject =
				slideContainer.querySelector('.image-object');
		...
		jQuery('.video-placeholder').on('click', function () {
			const videoElement = jQuery('#video-placeholder').get(0); // Get the native video element
		...
		const videoPlaceholderPause = document.querySelector(
			'.video_block_template_container_media_placeholder_action_btn'
		);
		const buttonText = document.querySelector(
			'.video_block_template_container_media_placeholder_action_btn .text'
...
// Pricing Block
document.addEventListener('DOMContentLoaded', function () {
	const pricingCalculator = document.querySelectorAll(
		'.pricing_calculator_template'
	);
	if (pricingCalculator.length) {
		class PricingAccordion {
			constructor(domNode) {
				...
				this.open =
					this.buttonEl.getAttribute('aria-expanded') === 'true';
		...
		function addData($this) {
			...
			const $targetenrollment = jQuery('.info_set_number');
			const benefitsData = jQuery(
				'.pricing_calculator_template_container_main_info_ul ul'
			);
			const disclaimerData = jQuery(
				'.pricing_calculator_template_container_main_pricing_disclaimer'
			);

			const currentPrice = parseInt($targetPrice.text(), 10);
			const currentEnrollment = parseInt($targetenrollment.text(), 10);
		...
		function collectAndAppendText() {
			...
			const textContent = getTextWithSpacing(container).trim(); // Trim the final string to remove any leading/trailing spaces

			const ariaReadElement = document.getElementById('aria-read');
			if (ariaReadElement) {
		...
		jQuery('.action, .pricing_calculator_accordion_add').on(
			'click',
			function () {
