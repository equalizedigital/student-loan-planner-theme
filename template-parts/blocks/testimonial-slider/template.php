...
<section id="<?php echo esc_attr( $classid ); ?>" class="<?php echo esc_attr( $class_name ); ?>">
	<div class="testimonial-slider-block-container">
		...
		<header class="testimonial-slider-block-container-header">
			<h2 class="testimonial-slider-block-container-header__title"><?php echo esc_html( $acf_title ); ?></h2>
		</header>
				...
				while ( have_rows( 'testimonials' ) ) :
					...
					<div class="testimonial-slider-block-container-testimonial-slider-testimonial ">
						<blockquote class="testimonial-slider-block-container-testimonial-slider-testimonial__content">
							...
							<span class="date">
								<span>
									<?php echo esc_html( get_field( 'location', $id_post ) ); ?>
								</span>
								<?php
								...
								?>
								<span>
									<?php echo esc_html( get_field( 'date', $id_post ) ); ?>
								</span>
							</span>
							<?php if ( ! empty( $rating ) ) : ?>
							<span class="rating">
		...
		<div class="testimonial-slider-block-container-testimonial_read_more">
			...
			<?php
			$more_link = get_field( 'link' );

			if ( $more_link ) {
				// The link field returns an array containing 'url', 'title', and 'target'.
				$url        = $more_link['url'];
				$more_title = $more_link['title'] ? $more_link['title'] : 'Default Title'; // Set a default title if none is provided.
				$target     = $more_link['target'] ? $more_link['target'] : '_self'; // Default target to '_self' if none is provided.

				echo '<a href="' . esc_url( $url ) . '" target="' . esc_attr( $target ) . '" class="btn btn-dark-bg">' . esc_html( $more_title ) . '</a>';
			}
			?>
		</div>
	</div>
